 ...
 */
package com.jess.arms.http.body;
...
public class ProgressResponseBody extends ResponseBody {
    ...
    public ProgressResponseBody(Handler handler, ResponseBody responseBody, List<WeakReference<ProgressListener>> listeners, int refreshTime) {
            ...
            public long read(Buffer sink, long byteCount) throws IOException {
                ...
                if (mListeners != null) {
                    ...
                    if (curTime - lastRefreshTime >= mRefreshTime || bytesRead == -1 || totalBytesRead == mProgressInfo.getContentLength()) {
                        ...
                        for (int i = 0; i < mListeners.length; i++) {
                            final ProgressListener listener = mListeners[i].get();
                            mHandler.post(new Runnable() {
                                @Override
                                public void run() {
